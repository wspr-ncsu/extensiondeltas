...
def dbScanTry1():
	...
	min_samples = [2]
	# hidSpecifics = ["klkkockojahojngfjllapnnelebkkepf", "bhoonbdhlkdbpodkaoehhgfnceacmipl"]
	# query0 = mongo.apiFreqStorageUpdOrderB.objects(sum0__gte=1)
	# query0 = mongo.allapib.objects(sumAll__gte=200)
	# query0 = mongo.allapib.objects(Q(sumAll__gte=30) & Q(sumAll__lte=200))
	# query0 = mongo.allapib.objects(Q(sumAll__gte=6) & Q(sumAll__lte=10))
	# query0 = mongo.allapib.objects(sumAll=5)
	query0 = mongo.allapib.objects(sumAll__gt=0)

	
	# query1 = mongo.allapib.objects(hid__in=hidSpecifics)
	print(len(query0))
	# print(len(query1))
	hids = []
	for each in query0:
		if each.hid not in hids:
			hids.append(each.hid)
	print(len(hids))
	'''
	...
	'''
	'''
	TODO: dice distance metric check
	TODO: mean silhouette score
	'''
	for eps_ in eps:
...
>>>>>>> Stashed changes
	for i in range(len(labels)):
		...
		print("unique hids = " + str(unique(hidsToPrint)))
		for (eachElement, eachFreq) in zip(clusterList, clusterListFreqs):
			print(eachElement)
		print("apis matched = " + str(sum(eachFreq)))
		# print(clusterList[:50])
		print("members in cluster = " + str(len(clusterList)))
